...
sce_ti <- readRDS(here("data", "slingshot_sce_v2.rds"))
srt_ti <- readRDS(here("data", "egfr_histo.rds"))
...
# srt_ti$lineage_assignment <- factor(srt_ti$lineage, levels = c("1", "1,2", "2"), labels = c("LUSC", "LUAD", "SCLC"))

srt_ti$mcluster_merge <- sce_ti$mcluster_merge

# srt_ti <- update_metadata(srt_ti, db$metadata, match_by = c("sample_id"))

sce_gam <- readRDS(here("data", "sce_gam_v2.rds"))
...
pathways_v2 <- read.table(here("data", "egfr_pathways.txt"), header = T, sep = "\t")
...
```{r}
quantile(srt_ti@meta.data[srt_ti$lineage_assignment == "LUAD", ]$pseudotime_scaled, prob = 0.95)

srt_ti@meta.data %>%
  filter(lineage_assignment == "LUAD") %>%
  ggplot(aes(x = pseudotime_scaled)) +
  geom_histogram(bins = 50) +
  geom_vline(xintercept = quantile(srt_ti@meta.data[srt_ti$lineage_assignment == "LUAD", ]$pseudotime_scaled, prob = c(0.025, 0.975)))
```
...
srt_ti@meta.data %>%
  ...
  geom_violin() +
  geom_hline(yintercept = quantile(srt_ti@meta.data[srt_ti$lineage_assignment == "LUAD", ]$pseudotime_scaled, prob = c(0.025, 0.975)))
```
...
anno_df$mapk_alt[!anno_df$has_impact | anno_df$time_point == "TN"] <- "NA"
...
srt_epi <- readRDS(here("data", "egfr_epi.rds"))
srt_meta <- readRDS(here("data", "egfr_cohort.rds"))@meta.data
