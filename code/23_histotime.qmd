...
```{r}
if (!file.exists(here("data", "ucell_scores_histotime.txt"))) {
  library(UCell)
  ...
  write.table(ucell_df, file = here("data", "ucell_scores_histotime.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
} else {
  ucell_df <- read.table(file = here("data", "ucell_scores_histotime.txt"), header = T, sep = "\t")
}
```
...
srt_ti@meta.data %>%
  ...
  geom_vline(xintercept = quantile(srt_ti@meta.data[srt_ti$lineage_assignment == "LUAD", ]$pseudotime_scaled, prob = c(0.05, 0.999)))
```
